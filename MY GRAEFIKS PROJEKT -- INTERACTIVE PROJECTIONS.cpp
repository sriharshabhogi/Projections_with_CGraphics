...
int perspective()
{
   ...
   for(i=0;i<12;i++)
   {
   	    ...
   	    //p1=multiply(p1,tr);
   	    //p2=multiply(p2,tr);
   	    s31=(p1.z*per.x-p1.x*per.z+zed*(p1.x-per.x))/(p1.z-per.z);
   	    w31=(p1.z*per.y-p1.y*per.z+zed*(p1.y-per.x))/(p1.z-per.z);
   	    s32=(p2.z*per.x-p2.x*per.z+zed*(p2.x-per.x))/(p2.z-per.z);
   	    w32=(p2.z*per.y-p2.y*per.z+zed*(p2.y-per.x))/(p2.z-per.z);
   	    p1.x=s31;
   	    p1.y=w31;
   	    p2.x=s32;
   	    p2.y=w32;
   ...
   while(opt==97||opt==100||opt==119||opt==115||opt==43||opt==45); //a d w s + - ke ascii values
   getch();
   return 0;
}
...
int main()
{
	...
	do  {
	outtextxy(220,120,"ORTHOGRAPHIC PROJECTION");	
	outtextxy(220,195,"PERSPECTIVE  PROJECTION");	
	outtextxy(220,270,"EXIT   THE  APPLICATION");	
	rectangle(165,100+y,465,150+y);
	opt=getch();
	if(opt==115)
	{
		y+=75;
